...
el.className = 'active'; // 이전 클래스가 모두 사라짐

// ✅ classList 방식: 기존 클래스를 유지하면서 추가
el.classList.add('active'); // 기존 클래스는 그대로
```
...
여러 요소를 한 번에 추가할 때, 하나씩 `appendChild`하면 리플로우(화면 재계산)가 여러 번 발생합니다.
`DocumentFragment`에 먼저 모아서 한 번에 삽입하면 리플로우가 1번만 발생합니다.
