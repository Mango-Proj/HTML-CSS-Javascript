...
DOM(ch01)을 배우면 화면을 **바꾸는** 방법을 알게 됩니다.  
하지만 **언제** 바꿔야 하는지는 이벤트가 알려줍니다.

- 버튼을 **클릭했을 때** 메뉴를 펼친다 → `click` 이벤트
- 검색창에 **글자를 입력할 때마다** 추천 검색어를 보여준다 → `input` 이벤트
- 폼 제출 버튼을 **클릭했을 때** 유효성 검사를 한다 → `submit` 이벤트
- 이미지 위에 **마우스를 올렸을 때** 설명 팝업을 띄운다 → `mouseenter` 이벤트

DOM과 이벤트는 항상 세트입니다. DOM으로 '무엇을 바꿀지' 정하고, 이벤트로 '언제 바꿀지' 정합니다.
...
> **비유:** 요리 레시피(HTML)에 요리사 지시사항(JS 동작)이 뒤섞여 있는 것과 같습니다.  
> 구조와 동작은 파일을 분리해서 관리하는 것이 좋습니다.
...
> **비유:** 전화기의 수신 담당자를 1명만 지정할 수 있는 것과 같습니다.  
> 새 담당자를 지정하면 이전 담당자는 자동으로 해제됩니다.
...
> **비유:** 같은 버튼에 "눌리면 LED 켜기", "눌리면 소리 재생" 두 가지를 모두 연결할 수 있는 것과 같습니다.
...
이벤트를 제거해야 하는 상황:
- 특정 조건이 충족된 후에는 더 이상 반응하지 않아야 할 때
- 컴포넌트가 화면에서 사라질 때 (메모리 누수 방지)
- 딱 1번만 실행되어야 할 때
...
> **비유:** 익명 함수는 주소도 이름도 없는 사람입니다. 처음에 등록했던 그 사람을 나중에 다시 찾을 수 없으므로 해제가 불가능합니다.
...
활용 예: 첫 방문 튜토리얼 팝업, 최초 1회 동의 버튼, 온보딩 가이드 등
...
**비유:** 응급실에 환자가 오면 접수 용지를 작성합니다.  
이름, 증상, 발생 시간, 위치 등이 적혀 있는 이 용지가 바로 이벤트 객체입니다.  
의사(핸들러 함수)는 이 용지를 보고 어떤 처치를 해야 할지 판단합니다.
...
> 실무 활용: 댓글 목록에서 각 댓글마다 리스너를 달지 않고, 목록 컨테이너 하나에만 리스너를 달아 처리합니다.  
> 새로 추가되는 댓글에도 자동으로 리스너가 작동합니다.
...
> **결론:** 이벤트 핸들러 안에서 `this`로 요소에 접근하고 싶다면 **화살표 함수를 쓰지 마세요.**  
> `e.currentTarget`을 사용하면 함수 형태에 관계없이 항상 안전합니다.
...
> **활용 예:** 드래그 앤 드롭, 커스텀 마우스 커서, 마우스 따라다니는 툴팁, 캔버스 그림 그리기
...
> `keypress`는 더 이상 사용하지 않는 오래된 이벤트입니다.  
> 특수 키(화살표, 백스페이스 등)를 감지하지 못하고, 브라우저 호환성도 낮습니다.  
> `keydown` 또는 `keyup`을 사용하세요.
...
> **비유:** `e.key`는 종이에 찍힌 글자, `e.code`는 키보드에서의 물리적 위치입니다.  
> 한국어 키보드와 영어 키보드는 같은 위치에 있어도 `e.key` 값이 다를 수 있습니다.  
> 게임 조작처럼 언어에 관계없이 같은 키를 인식해야 한다면 `e.code`를 사용하세요.
...
> **활용 예:** 트위터/X의 140자 제한 카운터, 댓글 입력창의 글자 수 표시

---

## 5. 폼 이벤트

폼은 로그인, 회원가입, 검색, 주문 등 **사용자로부터 정보를 입력받는 모든 곳**에 사용됩니다.
...
`submit` 이벤트의 핵심은 **`e.preventDefault()`** 입니다.

브라우저는 폼이 제출되면 기본적으로 **서버로 데이터를 전송하고 페이지를 새로고침**합니다.  
하지만 JavaScript로 유효성 검사를 하거나 비동기로 데이터를 보내려면 이 기본 동작을 막아야 합니다.
...
입력창을 클릭하면 테두리가 강조되는 효과, 많이 보셨죠?  
바로 `focus`와 `blur` 이벤트로 만드는 것입니다.
...
> **선택 기준:** 실시간 반응이 필요하면 `input`, 최종 확정 값만 처리하면 `change`를 사용합니다.  
> 드롭다운(select)이나 체크박스는 보통 선택 완료 후 한 번만 처리하면 되므로 `change`가 적합합니다.
